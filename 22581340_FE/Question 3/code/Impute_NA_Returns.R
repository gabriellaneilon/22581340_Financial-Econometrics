impute_missing_returns <- function(return_mat, impute_returns_method = "NONE"){
    ...
    if( impute_returns_method  == "Average") {
        ...
        return_mat <-
            return_mat %>% gather(Index, Returns, -date) %>%
            group_by(date) %>%
            mutate(Avg = mean(Returns, na.rm=T)) %>%
            mutate(Avg = coalesce(Avg, 0)) %>% # date with no returns - set avg to zero
            ungroup() %>%
            mutate(Returns = coalesce(Returns, Avg)) %>% select(-Avg) %>% spread(Index, Returns)
        ...
        if( impute_returns_method  == "Drawn_Distribution_Own") {
            ...
            return_mat <-
                ...
                left_join(return_mat %>% gather(Index, Returns, -date),
                          return_mat %>% gather(Index, Returns, -date) %>% group_by(Index) %>%
                ...
                select(-Random_Draws) %>% ungroup() %>% spread(Index, Returns)
            return(return_mat)
        } else

            if( impute_returns_method  == "Drawn_Distribution_Collective") {
                NAll <- nrow(return_mat %>% gather(Index, Returns, -date))
                # DIY: see what density function does
                return_mat <-
                    bind_cols(
                        return_mat %>% gather(Index, Returns, -date),
                    ...
                    mutate(Returns = coalesce(Returns, Random_Draws)) %>% select(-Random_Draws) %>% spread(Index, Returns)
                return(return_mat)
            } else
